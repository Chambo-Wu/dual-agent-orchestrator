...
> **当前定位**
>
> Dual Agent Orchestrator 已从最初的 planner / executor 双模型协作原型，演进为一个本地优先、可扩展、高度可观测的 Agent Workflow Runtime。当前系统已具备 job / workflow / goal 控制面、skill 运行与验证链路，并正在向 Goal Mode 完整实现和 Skill 自进化 v2 继续推进。
...
- [当前 Goal 任务路线](./docs/Goal任务-待完成路线分步实施计划-20260529.md)
- [Skill 自进化集成任务拆解](./docs/执行清单-Skill自进化集成任务拆解-20260529.md)
- [Deployment Validation 开发任务](./docs/执行清单-DeploymentValidation开发任务-20260529.md)
...
当前仓库中的 Skill 自进化能力已经不只是设计草案，而是进入了可用的 v1 落地状态。
...
- 已落地：Deployment Validation 已具备 deterministic isolated manifest replay、candidate workflow materialization、manual runtime replay validation，以及默认关闭的自动 pipeline runtime replay opt-in 开关 `skill_evolution.runtime_replay_in_auto_pipeline`
- 仍属 v1 启发式：auto proposal 目前生成的是最小安全候选变更，还不是高质量的结构化 skill rewrite
- 暂不建议广泛自治放开：自动 accept 仍受配置、风险分层、动态风险、稳定性与 validation readiness 共同约束
...
- `src/goal-timeline.ts`：goal timeline 渲染
